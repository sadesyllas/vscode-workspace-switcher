...
const mkdirp = __webpack_require__(5);
const process = __webpack_require__(6);
...
function saveWorkspacePrompt() {
    ...
    const directoryItems = workspaceEntryDirectories.map(directory => ({
        label: path_1.basename(directory),
        description: path_1.dirname(directory),
    }));
    const options = {
    ...
    vscode.window.showQuickPick(directoryItems, options).then((directoryItem) => {
        if (!directoryItem) {
            return;
        }
        vscode.window.showInputBox({
        ...
        }).then((workspaceFileName) => {
            workspaceFileName = (workspaceFileName || '').trim();
            if (workspaceFileName === '') {
                return;
            }
            workspaceFileName = workspaceFileName.replace(/\\+/g, '/').replace(/\/\/+/g, '/').replace(/^\//, '');
            workspaceFileName = path_1.join(...workspaceFileName.split(/\//));
            const workspaceDirectoryPath = path_1.join(directoryItem.description, directoryItem.label, path_1.dirname(workspaceFileName));
            workspaceFileName = path_1.basename(workspaceFileName);
            try {
            ...
            const workspaceFilePath = path_1.join(workspaceDirectoryPath, workspaceFileName) + '.code-workspace';
            const workspaceFolderPaths = (vscode.workspace.workspaceFolders || []).map((workspaceFolder) => ({ path: workspaceFolder.uri.fsPath }));
            ...
            const workspaceFilePathSaveFunc = () => {
                try {
                    fs_1.writeFileSync(workspaceFilePath, workspaceFileContent, { encoding: 'utf8' });
                    switchToWorkspace({
                        path: workspaceFilePath,
                    });
                }
                catch (error) {
                    vscode.window.showErrorMessage('Error while trying to save workspace '
                        + `${workspaceFileName} to ${workspaceFilePath}: ${error.message}`);
                }
            };
            if (fs_1.existsSync(workspaceFilePath)) {
                vscode.window.showInformationMessage(`File ${workspaceFilePath} already exists. Do you want to override it?`, 'Yes', 'No').then((answer) => {
...
function switchWorkspacePrompt(inNewWindow = false) {
    ...
    const workspaceItems = workspaceEntries.map(entry => ({
        label: entry.name,
    ...
    vscode.window.showQuickPick(workspaceItems, options).then((workspaceItem) => {
        if (!workspaceItem) {
            return;
        }
...
function switchToWorkspace(workspaceEntry, inNewWindow = false) {
    const app = getApp();
    const command = `${app} ${inNewWindow ? '-n' : '-r'} "${workspaceEntry.path}"`;
    child_process_1.exec(command, onCommandRun);
}
function getWorkspaceEntryDirectories() {
    var paths = vscode.workspace.getConfiguration('vscodeWorkspaceSwitcher').get('paths');
    ...
    const pathsAfterGlobbingHash = uniquePaths
        ...
        .filter(p => {
        try {
            return fs_1.existsSync(p) && fs_1.statSync(p).isDirectory();
        }
        catch (err) {
            return false;
        }
...
function gatherWorkspaceEntries() {
    const directoryPaths = getWorkspaceEntryDirectories();
    return directoryPaths.reduce((acc, dir) => {
        return fs_1.readdirSync(dir)
            .filter(fileName => {
            try {
                return /.code-workspace$/.test(fileName) && fs_1.statSync(path_1.join(dir, fileName)).isFile();
            }
            catch (err) {
                return false;
            }
        })
            .reduce((accProxy, fileName) => {
            accProxy.push({
...
function getFirstWorkspaceFolderName() {
...
function getApp() {
    ...
    const app = vscode.workspace.getConfiguration('vscodeWorkspaceSwitcher').get(key);
    if (app.search(/\s/) !== -1) {
        return `"${app}"`;
    }
    if (app === 'code' && process.platform.toLocaleLowerCase().startsWith("win")) {
        const codeWindowsScriptPath = path_1.join(path_1.dirname(process.execPath), 'bin', 'code.cmd');
        if (fs_1.existsSync(codeWindowsScriptPath) && fs_1.statSync(codeWindowsScriptPath).isFile()) {
            return `"${codeWindowsScriptPath}"`;
        }
    }
    return app;
}
function onCommandRun(err, stdout, stderr) {
    if (err || stderr) {
...
var omit = __webpack_require__(8);
var visit = __webpack_require__(12);
var Emitter = __webpack_require__(14);
var exclude = __webpack_require__(15);
var include = __webpack_require__(59);
var npm = __webpack_require__(60);
var symlinks = __webpack_require__(61);
var iterators = __webpack_require__(70);
var Handler = __webpack_require__(78);
var Pattern = __webpack_require__(80);
var options = __webpack_require__(82);
var readers = __webpack_require__(90);
...
var lazy = __webpack_require__(72)(__webpack_require__(93));
...
var omit = __webpack_require__(8);
var unique = __webpack_require__(21);
var cache = __webpack_require__(22);
var isGlob = __webpack_require__(25);
...
micromatch.any       = any;
micromatch.braces    = micromatch.braceExpand = __webpack_require__(30);
...
var path = __webpack_require__(2);
var fileRe = __webpack_require__(28);
...
var braces = __webpack_require__(30);
var brackets = __webpack_require__(46);
var extglob = __webpack_require__(48);
var parse = __webpack_require__(50);
var chars = __webpack_require__(55);
...
var repeat = __webpack_require__(44);
var tokens = __webpack_require__(45);
...
var repeatStr = __webpack_require__(43);
var repeat = __webpack_require__(44);
...
var mathRandom = __webpack_require__(41);
...
var isPosixBracket = __webpack_require__(47);
...
var isExtglob = __webpack_require__(49);
var re, cache = {};
...
var isGlob = __webpack_require__(51);
var findBase = __webpack_require__(52);
var extglob = __webpack_require__(49);
var dotfile = __webpack_require__(54);
...
if (/\bgfs4\b/i.test(process.env.NODE_DEBUG || '')) {
  process.on('exit', function() {
    debug(queue)
    __webpack_require__(69).equal(queue.length, 0)
  })
}
...
var lazy = __webpack_require__(72)(__webpack_require__(74));
...
var warning = lazyCache(__webpack_require__(73))('ansi-yellow');
...
var isObject = __webpack_require__(58);
var forIn = __webpack_require__(76);
...
var util = __webpack_require__(68);
var Emitter = __webpack_require__(14);
var define = __webpack_require__(79).defineProp;
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_RESULT__;/*! is-windows v0.1.0 | MIT LICENSE (c) 2015 | https://github.com/jonschlinkert/is-windows */
...
var utils = __webpack_require__(88)(__webpack_require__(89));
...
var path = __webpack_require__(2);
var relative = __webpack_require__(92);
